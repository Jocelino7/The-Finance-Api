...
describe("transaction tests", () => {
    ...
    it("given an userId and a month it should return a report object", async () => {
        fakeTransaction = {...transactionMocks[5],transactionDate:{
            ...transactionMocks[5].transactionDate,
            day:22,
            month:4
        }}
        const fakeTransaction2 ={...transactionMocks[6],transactionDate:{
            ...transactionMocks[6].transactionDate,
            month:4,
            day:22
        }}
        const fakeToken = generateFakeToken()
        ...
        await addTransaction(fakeTransaction2, req)
       //get report
       const result = await req.post(`${baseUrl}report/${fakeTransaction.user._id}/4`)
       .set("authorization", fakeToken)
       console.log("test-"+JSON.stringify(result.body))

       expect(result.body).not.toBeNull()
       expect(result.body).toBeDefined()
       expect(result.body.total).toBeGreaterThan(0)
       expect(result.body.month).toBe(4)
       expect(result.body.week4.transaction.length).toBe(2)
