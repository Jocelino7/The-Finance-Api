...
import { CategoryType, User } from "../../dtos/dto"
...
export interface CategoryRepository {
    ...
    search(value:string,user:User):Promise<CategoryType[]>,
...
class CategoryRepositoryImpl implements CategoryRepository {
    ...
    async search(value: string, user: User): Promise<CategoryType[]> {
        try {
            const categories = await this.categoryModel.find({"user._id":user._id, $or:[
                {
                    name:{RegExp:value}
                }
            ]})
