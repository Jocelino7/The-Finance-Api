...
import { GoalType, User } from "../../dtos/dto"
...
export interface GoalRepository {
    ...
    search(value: string, user: User): Promise<GoalType[]>,
...
class GoalRepositoryImpl implements GoalRepository {
    ...
    async search(value: string, user: User): Promise<GoalType[]> {
        try {
            const goals = await this.goalModel.find({
                _id: user._id, $or: [{
                    name: { RegExp: value },
                    description: { RegExp: value }
                }]
            })
            return goals
        }
